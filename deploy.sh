...
sed -e "s/\$SERVICE_NAME/$SERVICE_NAME/g" ./deployment/config.$ENVIROMENT.yaml > ./deployment/k8s-config.yaml
kubectl apply -f ./deployment/k8s-main.yaml -n $NAMESPACE
kubectl apply -f ./deployment/k8s-config.yaml -n $NAMESPACE
if [[ $? != 0 ]]; then exit 1; fi
